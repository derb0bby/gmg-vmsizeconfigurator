...
const VMRecommendation: React.FC<VMRecommendationProps> = ({
  ...
  if (selectedApplications.length === 0) {
    return (
      <div className="mt-8">
        <p className="text-gray-600">Select an application to get a hardware configuration recommendation.</p>
      </div>
    );
  }
      ...
      case "color-proof":
        return [
          ...defaultFAQs,
          {
            ...
            content:
              "ColorProof requires a USB port for connecting to color measurement devices. The VM must have USB passthrough capability enabled. For optimal color accuracy, we recommend using a VM with dedicated GPU resources when working with high-resolution images or when color accuracy is critical for your workflow.",
          },
        ];
    ...
    <div className="mt-8">
      ...
      {alternativeVMs.length > 0 && (
        <div className="mb-6">
          ...
          <div className="space-y-4">{alternativeVMs.map((vm, index) => renderVMCard(vm))}</div>
        </div>
      )}
